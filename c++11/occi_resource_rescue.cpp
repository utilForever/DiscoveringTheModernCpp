...
{
	result_set_deleter(shared_ptr<Connection> conn, Statement* stmt) : conn(conn), stmt(stmt) {}
...
public:
	...
	db_manager(string const& dbConnection, string const& dbUser, string const& dbPw) :
...
// 오라클 쿼리, c++03/occi_old_style.cpp 참조